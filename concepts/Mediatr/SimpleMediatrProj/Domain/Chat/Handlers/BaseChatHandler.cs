...
{
    public abstract class BaseChatHandler<TRequest, TOutput> : IRequestHandler<TRequest, TOutput> where TRequest : IRequest<TOutput>
    {
        protected abstract Task<TOutput> Handle(TRequest request);
        ...
        public Task<TOutput> Handle(TRequest request, CancellationToken cancellationToken) => Handle(request);
    }
}
